...
本项目构建英国公共政策 Query Planner 所需的知识库、官方评测集、分层 SFT、单跳/多跳 DPO、单跳/多跳 GRPO-ready 数据，以及可选的火山方舟生成接口。已完成 DPO 与 GRPO 训练和正式评测。
...
统一评测基础 SFT、冷启动、DPO 和 GRPO 四个检查点，用于分别衡量多跳冷启动的独立贡献，以及两种后训练方法相对同一共享检查点的增益。
